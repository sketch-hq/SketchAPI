;(function () {
  ...
  function generateToc($content) {
    ...
    h1AndH2.each(function(i, h1OrH2) {
      ...
      if (isH1) {
        if (i !== 0) {
          if (h1AndH2[i - 1].tagName === 'H2') { // previous tag was an h2
      ...
      if (isH1) {
        htmlContent += '<li><a href="#' + h1OrH2.id + '" class="toc-h1 toc-link" data-title="' + h1OrH2.textContent + '">' + h1OrH2.textContent + '</a>'
      } else {
        if (h1AndH2[i -1].tagName === 'H1') { // previous tag was an h1
          htmlContent += '<ul class="toc-list-h2">' // we need to open an ul for the h2
        }
        htmlContent += '<li><a href="#' + h1OrH2.id + '" class="toc-h2 toc-link" data-title="' + h1OrH2.textContent + '">' + h1OrH2.textContent + '</a></li>'
      }
    });
    if (h1AndH2[h1AndH2.length - 1].tagName === 'H2') { // latest tag was an h2
      htmlContent += '</ul>' // we need to close the h2's ul
    }
